...
const divStr = (number1, number2) => {
  ...
  if (
    ...
    tg = number1.substring(0, number2.length);
    for (let i = number2.length - 1; i < number1.length; i++) {
      count = 0;
      tg += number1[i];
      while (tg.length > 1 || (tg.length === number2.length && tg >= number2)) {
        count++;
        tg = subStr(tg, number2);
      }
      div += count.toString();
    }
    mod = tg;
    while (mod.length > 1 && mod.charAt(0) === "0") {
      mod = mod.substring(1);
    }
  }
  console.log(div);
