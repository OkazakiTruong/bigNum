...
const sumSalary = (listOfficers) => {
  ...
  listOfficers.forEach((item) => {
    if (monthSelect.value === "0") {
      ...
      if (String(item.Month) === monthSelect.value) {
        result = add(result, numInput(String(item.Salary)));
      }
    }
  });
...
const averageSalary = (listOfficers) => {
  ...
  if (monthSelect.value === "0") {
    result = divInt(result, numInput(String(listOfficers.length)));
  } else {
    length = 0;
    listOfficers.forEach((item) => {
      if (String(item.Month) === monthSelect.value) {
    ...
    result = divInt(result, numInput(String(length)));
  }
  return result;
};
...
btnSum.addEventListener("click", () => {
  ...
  if (listOfficers) {
    ...
    let html = `Tổng lương ${
      monthSelect.value !== "0" ? `tháng ${monthSelect.value}` : ""
    } là: <span class="salary-show">${convertToSalary(
      stringInput(result)
    )}</span> đồng`;
...
const renderTableRows = (table, row, index) => {
  ...
  <td>${row.Address}</td>
  <td>${row.Phone}</td>
  <td>${convertToSalary(String(row.Salary))}</td>
  <td>${row.Month}</td>
