...
if (fileList?.length) {
  ...
  console.log(pagination(1, totalPage));

  const renderStart = current > 1 ? current * page_limit + 1 : current;
  const renderLength =
    fileList.length - renderStart >= 5
      ? page_limit
      : fileList.length - renderStart;
  for (let i = renderStart - 1; i < renderLength - 1; i++) {
