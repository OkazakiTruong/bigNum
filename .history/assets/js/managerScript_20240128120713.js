...
if (fileList?.length) {
  ...
  console.log(pagination(6, totalPage));

  const renderStart = current*page_limit;
  const renderLength = (fileList.length - renderStart)>5?:;
  fileList.map((row, index) => {
    renderTableRows(tableOfficer, row, index,current);
  });
} else {
