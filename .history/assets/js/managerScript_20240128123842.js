...
const renderPagination = (pagination, active) => {
  return pagination
    .map((item) => {
      if (item === active) {
        return `<span class="pagination-item active">${item}</span>`;
      }
      return `<span class="pagination-item">${item}</span>`;
    })
    .join("");
};
...
if (fileList?.length) {
  ...
  paginationEle.innerHTML = renderPagination(
    pagination(current, fileList.length),
    current
  );
